...
class Maze:
    ...
    def eat_dots(self, x, y):
        """Check for Pacman eating dots"""
        for dot_list in [self.dots.top_row, self.dots.bottom_row,
                         self.dots.left_col, self.dots.right_col]:
            for dot in dot_list:
                if Maze.dist(x, y, dot.x, dot.y) < self.dots.EAT_DIST:
                    self.dots.eat_dot(dot)
                    return  # Assuming only one dot can be eaten at a time
